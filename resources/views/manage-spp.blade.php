                ...
                <div class="data-control-show">
                    ...
                    <div class="data-control-show-select gap-2">
                        ...
                        <div class="d-flex gap-2">
                            <select name="tahun-ajaran" id="tahun-ajaran" class="form-select form-select-sm tahun-select">
                            ...
                            <tbody class="fw-medium">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="#" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#editModal{{ $item->kode_pembayaran }}">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="#" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $item->kode_pembayaran }}">
                                    ...
                                    <td class="text-center">
                                        <form method="POST" action="{{ route('addToSpesificKelas', $item['kode_pembayaran']) }}">
                                            @csrf
                                            <div class="d-flex gap-2">
                                                <select name="id_kelas" id="entries" class="form-select form-select-sm" style="width: 70px; height: 30px;">
                                                    @foreach ($kelas as $k)
                                                ...
                                                <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-add"></i></button>
                                            </div>
                                        </form>
                                    </td>
                                </tr>
                                ...
                                <div class="modal fade" id="editModal{{ $item->kode_pembayaran }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->kode_pembayaran }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModalLabel{{ $item->kode_pembayaran }}">Edit Data
                                                    Siswa</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <!-- Isi form edit di sini -->
                                                <form method="POST" action="{{ route('adminSpp.update', ['kode_pembayaran' => $item->kode_pembayaran]) }}">
                                                    @csrf
                                                    @method('PUT')
                                ...
                                <!-- Modal delete -->
                                @foreach ($spp as $index => $item)
                                <div class="modal fade" id="deleteModal{{ $item->kode_pembayaran }}" tabindex="-1" aria-labelledby="deleteModalLabel{{ $item->kode_pembayaran }}" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="deleteModalLabel{{ $item->kode_pembayaran }}">Hapus Data Siswa</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Apakah Anda yakin ingin menghapus data SPP dengan kode {{ $item->kode_pembayaran }}?</p>
                                            </div>
                                            <div class="modal-footer">
                                                <form method="POST" action="{{ route('adminSpp.delete', ['kode_pembayaran' => $item->kode_pembayaran]) }}">
                                                    @csrf
                                                    @method('DELETE')
